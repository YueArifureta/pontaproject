...
const VALID_INTERACTIVE_HEADER_TYPES = ['imageMessage', 'videoMessage', 'documentMessage']
const VALID_CAROUSEL_HEADER_TYPES = ['imageMessage', 'videoMessage']

const hasValidInteractiveHeader = (m: any): boolean =>
	Object.keys(m).some(k => VALID_INTERACTIVE_HEADER_TYPES.includes(k))

const hasValidCarouselHeader = (m: any): boolean =>
	Object.keys(m).some(k => VALID_CAROUSEL_HEADER_TYPES.includes(k))

const prepareNativeFlowButtons = (message: any) => {
	const buttons = message.nativeFlow
	const isArray = Array.isArray(buttons)
	const correctedField = isArray ? buttons : buttons?.buttons || []
	const messageParamsJson: any = {}
...
export const generateWAMessageContent = async (
	...
	if (hasNonNullishProperty(message, 'raw' as any)) {
		m = (message as any).raw
	} else if (hasNonNullishProperty(message, 'buttons' as any)) {
		const msg = message as any
		const buttonsMessage: any = {
			buttons: msg.buttons.map((button: any) => {
				const buttonText = button.text || button.buttonText
				if (hasOptionalProperty(button, 'sections')) {
					return {
						nativeFlowInfo: { name: 'single_select', paramsJson: JSON.stringify({ title: buttonText, sections: button.sections }) },
		...
		if (hasOptionalProperty(msg, 'text')) {
			buttonsMessage.contentText = msg.text
			buttonsMessage.headerType = ButtonHeaderType.EMPTY
		} else {
			if (hasOptionalProperty(msg, 'caption')) buttonsMessage.contentText = msg.caption
			const type = Object.keys(m)[0]?.replace('Message', '').toUpperCase()
			buttonsMessage.headerType = (ButtonHeaderType as any)[type!]
			Object.assign(buttonsMessage, m)
		}
		if (hasOptionalProperty(msg, 'footer')) buttonsMessage.footerText = msg.footer
		m = { buttonsMessage }
	} else if (hasNonNullishProperty(message, 'sections' as any)) {
		const msg = message as any
		m = {
			listMessage: {
	...
	} else if (hasNonNullishProperty(message, 'templateButtons' as any)) {
		const msg = message as any
		const hydratedTemplate: any = {
			hydratedButtons: msg.templateButtons.map((button: any, i: number) => {
				const buttonText = button.text || button.buttonText
		...
		if (hasOptionalProperty(msg, 'text')) {
			...
			if (hasOptionalProperty(msg, 'caption')) {
				hydratedTemplate.hydratedTitleText = msg.title
				hydratedTemplate.hydratedContentText = msg.caption
			}
			Object.assign(hydratedTemplate, m)
		}
		if (hasOptionalProperty(msg, 'footer')) hydratedTemplate.hydratedFooterText = msg.footer
	...
	} else if (hasNonNullishProperty(message, 'nativeFlow' as any)) {
		const msg = message as any
		const interactiveMessage: any = { nativeFlowMessage: prepareNativeFlowButtons(msg) }
		if (hasOptionalProperty(msg, 'bizJid')) {
			interactiveMessage.collectionMessage = { bizJid: msg.bizJid, id: msg.id, messageVersion: 1 }
		} else if (hasOptionalProperty(msg, 'shopSurface')) {
		...
		if (hasOptionalProperty(msg, 'text')) {
			...
			if (hasOptionalProperty(msg, 'caption')) {
				...
				interactiveMessage.header = { title: msg.title || '', subtitle: msg.subtitle || '', hasMediaAttachment: isValidHeader }
				interactiveMessage.body = { text: msg.caption }
			}
			if (hasOptionalProperty(msg, 'thumbnail') && !!msg.thumbnail) interactiveMessage.jpegThumbnail = msg.thumbnail
			Object.assign(interactiveMessage.header || {}, m)
		}
		if (hasOptionalProperty(msg, 'audioFooter')) {
		...
		} else if (hasOptionalProperty(msg, 'footer')) {
			interactiveMessage.footer = { text: msg.footer }
		}
		m = { interactiveMessage }
	} else if (hasNonNullishProperty(message, 'cards' as any)) {
		const msg = message as any
		const interactiveMessage: any = {
			carouselMessage: {
				cards: await Promise.all(msg.cards.map(async (card: any) => {
					let carouselHeader: any = {}
					...
					const carouselCard: any = { nativeFlowMessage: prepareNativeFlowButtons(card.nativeFlow ? card : []) }
		...
		if (hasOptionalProperty(msg, 'text')) interactiveMessage.body = { text: msg.text }
		if (hasOptionalProperty(msg, 'footer')) interactiveMessage.footer = { text: msg.footer }
		m = { interactiveMessage }
	} else if (hasNonNullishProperty(message, 'richResponse' as any) || hasNonNullishProperty(message, 'code' as any) || hasNonNullishProperty(message, 'table' as any)) {
		m = prepareRichResponseMessage(message as any) as any
	}

	// interactiveAsTemplate wrapper
	if (hasOptionalProperty(message, 'interactiveAsTemplate' as any) && !!(message as any).interactiveAsTemplate) {
